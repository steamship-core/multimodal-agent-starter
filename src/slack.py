...
class SlackElement(BaseModel):
    ...
    text: Optional[str] = Field(
        description="The text of the element. Does not contain the user"
    )
...
class SlackEvent(BaseModel):
    ...
    def is_message(self) -> bool:
        ...
        return self.type and (
            self.type.startswith("message") or self.type == "app_mention"
        )
...
class SlackRequest(BaseModel):
    ...
    type: Optional[str] = Field(
        description="The type of inbound request"
    )  # event_callback
...
class SlackTransportConfig(Config):
    ...
    slack_api_base: str = Field(
        SLACK_API_BASE,
        description="Slack API base URL. If blank defaults to production Slack.",
    )
...
class SlackTransport(Transport):
    ...
    def _manifest(self) -> dict:
        ...
        invocable_instance_handle = (
            self.agent_service.context.invocable_instance_handle
            or "Development Steamship Agent"
        )
        invocable_handle = (
        ...
        return {
            ...
            "features": {
                "bot_user": {"display_name": invocable_handle, "always_online": True}
            },
            "oauth_config": {
                "scopes": {
                    "bot": [
    ...
    def _send(self, blocks: List[Block], metadata: Metadata):  # noqa: C901
        ...
        if not bot_token:
            logging.error(
                "Unable to send Slack Message: Slack transport had null bot token"
            )
            return
    ...
    def set_slack_access_token(self, token: str) -> InvocableResponse[str]:
        ...
        kv = KeyValueStore(
            client=self.agent_service.client, store_identifier=SETTINGS_KVSTORE_KEY
        )
